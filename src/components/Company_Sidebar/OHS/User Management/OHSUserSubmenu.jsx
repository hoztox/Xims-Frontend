...
const OHSUserSubmenu = () => {
    ...
    const handleCategoryClick = (category) => {
        if (props && props.handleItemClick) {
            ...
            props.handleItemClick(category.id, category.path, "ohsuser");
        } else {
        ...
        <div className="grid grid-cols-2 gap-[10px] bg-[#1C1C24] p-5 w-[380px] absolute top-16 border-t border-r border-b border-[#383840]">
            {categories.map((category) => {
                ...
                return (
                    <div
                        key={category.id}
                        className="flex flex-col items-center justify-center py-[10px] rounded-md bg-[#F9291F10] transition-colors duration-200 cursor-pointer w-[165px] h-[100px] gap-[10px] documentation-submenu-cards"
