...
const AMSPolicy = () => {
    ...
    const [AmsPolicies, setAmsPolicies] = useState([
        { id: 1, name: 'Energy Policy' },
    ]);

    const handleAddAMSpolicy = () => {
        navigate('/company/ams/addpolicy')
    }
    ...
    return (
        <div className="bg-[#1C1C24] rounded-lg text-white p-5">
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <button className="bg-transparent border border-[#DD6B06] text-[#DD6B06] rounded-[4px] p-[10px] flex items-center justify-center gap-[10px] transition-all duration-200 w-[140px] h-[42px] add-policy-btn hover:bg-[#DD6B06] hover:text-white group"
                    onClick={handleAddAMSpolicy}
                >
                    <span>Add Policy</span>
                    <Plus size={22} className='text-[#DD6B06] group-hover:text-white transition-colors duration-200' />
                </button>
            </div>
            ...
            <div className="bg-[#24242D] rounded-md overflow-hidden">
                ...
                <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                    {AmsPolicies.map((amsPolicy) => (
                        <div key={amsPolicy.id} className="px-5 pt-6 pb-5 border-b border-gray-700 flex justify-start items-center last:border-b-0">
                            <div className="flex items-center gap-[50px]">
                                <div className='gap-[15px] flex flex-col'>
                                    <span className="policy-name text-[#DD6B06]">{amsPolicy.name}</span>
